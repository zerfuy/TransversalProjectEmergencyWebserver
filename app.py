...
def index():
    ...
    for ft in fromTo:
        ...
        if "routes" in requests.get(link).json():
            # ctypes.windll.user32.MessageBoxW(0, "updating", "Your title", 1)
    ...
    cur.close()
    return render_template('index.html', fire_engines=fire_engines_pos, routingInfo=routingInfo, fires=fire_pos)